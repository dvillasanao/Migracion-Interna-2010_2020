...
tabla <- Migrantes %>%
          as.data.frame() %>%
           expss::cross_cases(ENT, ENT_PAIS_NAC, weight = Freq) %>%
            as.data.frame() %>%
             slice(-33) %>% 
              select(-row_labels) %>%
...
circos.clear()
pdf(paste0(here::here(),"/Graficos/Estado/02_Lugar de nacimiento/2020/ChordDiagram de MNac a nivel estatal.pdf"), width = 20, height = 20)
