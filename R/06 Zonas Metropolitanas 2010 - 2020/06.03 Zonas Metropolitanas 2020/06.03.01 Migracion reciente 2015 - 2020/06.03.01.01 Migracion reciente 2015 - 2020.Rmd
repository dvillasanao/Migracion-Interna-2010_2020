        ...
        mutate(I_ZM = case_when(.$CVE_ZM == .$CVE_ZM_RES ~ "Pertenecen a la Zona Metropolitana",
         ...
         filter(EDAD >= 5 & EDAD <= 130) %>%
          filter(CVE_MUN_RES %in% municipios & 
                 CVE_MUN %in% ZM_2020$CVE_MUN # Todos los municipios que pertenecen a la zona metropolitana
                 ) %>%
...
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/ChordDiagram de MR5a desagregado por ZM (Intramunicipal).pdf"), width = 25, height = 25)
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/ChordDiagram de MR5a de ZMVM_grupos (Intramunicipal).pdf"), width = 18, height = 15)
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
```{r, echo = FALSE}
save(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Indicadores de MR5a por ZM 2020 (Intramunicipal).RData"))
              ...
              mutate_if(is.factor, as.character) %>%
               mutate(value = ifelse((.$rn != .$cn) & (.$rn %in% ZM | .$cn %in% ZM), value, 0)) %>% 
                mutate(rn = case_when(.$rn %in% ZM & .$rn %in% filtro  ~ .$rn,
                                      ...
                                      .$rn %nin% ZM ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$rn, 1, 3))]), 20)),
...
circos.clear()
pdf(paste0(here::here(), "/Graficos/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/ChordDiagram de MR5a de ZMVM_grupos (Intermunicipal).pdf"), width = 18, height = 15)
circos.par(start.degree = 90, 
           gap.degree = 3, 
           track.margin = c(-0.2, 0.2), 
           points.overflow.warning = FALSE)
...
```{r, echo = FALSE}
save(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Indicadores de MR5a por ZM 2020 (Intermunicipal).RData"))
