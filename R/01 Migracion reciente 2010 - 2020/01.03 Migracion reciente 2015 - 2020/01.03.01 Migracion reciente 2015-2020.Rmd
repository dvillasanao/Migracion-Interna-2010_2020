              ...
              as.data.frame() %>%
               mutate(EDAD = as.numeric(.$EDAD)) %>%
                subset(EDAD >= 5 & EDAD <= 130) %>%
                 filter(ENT_PAIS_RES_5A %in% tablas)  # Filtro del lugar de residencia dentro del país. 
```
...
p
require(htmlwidgets)
htmlwidgets::saveWidget(p, paste0(here::here(), "/Graficos/Estado/01_Migracion reciente/2020/MR5a a nivel estatal 2010 - 2015.html"), selfcontained = TRUE)
#require(webshot)
#webshot(url = paste0(here::here(), "/Graficos/Estado/01_Migracion reciente/2020/MR5a a nivel estatal 2010 - 2015.html"),
 #         file = paste0(here::here(), "/Graficos/Estado/01_Migracion reciente/2020/MR5a a nivel estatal 2010 - 2015.png"),
  #                cliprect = "viewport")
```
...
#https://stackoverflow.com/questions/58171109/how-change-the-numeric-format-of-axis-on-r-chord-diagram-to-1k-2k-etc
#https://stackoverflow.com/questions/72861657/fix-axis-numbering-in-circlize-chord-diagram-and-font-issues
require(circlize)
...
pdf(paste0(here::here(), "/Graficos/Estado/01_Migracion reciente/2020/ChordDiagram de MR5a a nivel estatal.pdf"), width = 20, height = 20)
...
pdf(paste0(here::here(), "/Graficos/Estado/01_Migracion reciente/2020/ChordDiagram de MR5a desagregado por estado.pdf"), width = 20, height = 20)
for(i in 1:32){
circos.clear()
circos.par(start.degree = 90, 
           gap.degree = 3, 
...
```{r, eval = FALSE}
# Se cambian las etiquetas de los estados
load(file = paste0(here::here(), "/Bases/Estado/01_Migracion reciente/2020/Matriz de migracion reciente a nivel estatal 2015-2020.RData"))
...
#write.xlsx(p, file = "Output/Estado/01_Migracion reciente/2020/Indicadores de migracion reciente a nivel estatal 2015 - 2020.xlsx", overwrite = TRUE)
```
...
write.xlsx(tabla, file = paste0(here::here(),"/Output/Estado/01_Migracion reciente/2020/Indicadores de migracion reciente a nivel estatal 2015 - 2020.xlsx"), overwrite = TRUE)
save(tabla, file = paste0(here::here(), "/Output/Municipio/01_Migracion reciente/2020/Indicadores de migracion reciente a nivel estatal 2015 - 2020.RData"))
```

```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Municipio/01_Migracion reciente/2020/Indicadores de migracion reciente a nivel estatal 2015 - 2020.RData"))
...
tabla1 <-  lapply(9, function(x){
                         ...
                         # Se reestructura la clave de municipios y se seleccionan solo los municipios del filtro del estado
                          mutate(rn = case_when(.$rn %in% filtro ~ paste0(estados[x], " Otros municipios"), 
                                                ...
                                                substr(.$rn, 1, 3) %nin% estados[x] ~  str_wrap(paste0(nom_estados[as.numeric(substr(.$rn, 1, 3))]), 30)),
                                 cn = case_when(.$cn %in% filtro ~ paste0(estados[x], " Otros municipios"), 
                                                substr(.$cn, 1, 3) %in% estados[x] & .$cn %nin% filtro ~ .$cn, 
                                                substr(.$cn, 1, 3) %nin% estados[x] ~ str_wrap(paste0(nom_estados[as.numeric(substr(.$cn, 1, 3))]), 30)))  %>%
...
saveRDS(tabla1, file = paste0(here::here(), "/Output/Municipio/01_Migracion reciente/2020/Paleta de colores MR5a a nivel municipal.RDS"))
```


```{r}
pdf(paste0(here::here(), "/Graficos/Municipio/01_Migracion reciente/2020/ChordDiagram de MR5a desagregado por estado.pdf"), width = 20, height = 20)
for(i in 1){
...
```{r, echo = FALSE}
save(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Indicadores de MR5a a nivel intramunicipal 2020.RData"))
...
```{r, echo = FALSE}
save(file = paste0(here::here(), "/Output/Municipio/06_Zonas Metropolitanas/2020/01_Migracion reciente/Indicadores de MR5a a nivel intermunicipal 2020.RData"))
