              ...
              as.data.frame() %>%
               mutate(EDAD = as.numeric(.$EDAD)) %>%
                subset(EDAD >= 5 & EDAD <= 130) %>%
                 filter(ENT_PAIS_RES10 %in% tablas)  # Filtro del lugar de residencia dCVE_ENTro del país. 
```
...
**Matriz de migración reciente hace 5 años a nivel estatal, 2015 - 2015**
...
MC <- mydata  %>%
       select(FACTOR, ESTRATO, UPM, CVE_ENT, ENT_PAIS_RES10, EDAD) %>%
        as.data.frame() %>%
         mutate(EDAD = as.numeric(.$EDAD)) %>%
          filter((EDAD >= 3 & EDAD <= 130))  %>%
           mutate(ENT_PAIS_RES10 = case_when(.$ENT_PAIS_RES10 %in% tablas ~.$ENT_PAIS_RES10,
                                             .$ENT_PAIS_RES10 %nin% tablas ~ "888", #Residencia en otro país
...
save(Migrantes, file = paste0(here::here(), "/Output/Estado/01_Migracion reciente/2015/Matriz de migracion reciente a nivel estatal 2015.RData"))
```
...
write.xlsx(tabla, file = paste0(here::here(), "/Output/Estado/01_Migracion reciente/2015/Indicadores de migracion reciente a nivel estatal 2010 - 2015.xlsx"), overwrite = TRUE)

save(tabla, file = paste0(here::here(), "/Output/Estado/01_Migracion reciente/2015/Indicadores de migracion reciente a nivel estatal 2010 - 2015.RData"))
```

```{r, echo = FALSE}
load(file = paste0(here::here(), "/Output/Estado/01_Migracion reciente/2015/Indicadores de migracion reciente a nivel estatal 2010 - 2015.RData"))
...
Pob.5ymas <- mydata %>%
              mutate(ENT_MUN_RES_2010 = paste0(ENT_PAIS_RES10, MUN_RES10)) %>%
               mutate(ENT_PAIS_RES10 = case_when(.$ENT_PAIS_RES10 %in% estados ~.$ENT_PAIS_RES10,
                                                 ...
                                                 .$ENT_PAIS_RES10 %in% "998" ~ "998",
                                                 .$ENT_PAIS_RES10 %in% "997" ~ "999"),
                      CVE_MUN_RES = case_when(.$ENT_MUN_RES_2010 %in% municipios ~.$ENT_MUN_RES_2010,
                                                nchar(.$ENT_MUN_RES_2010) == 3 ~ "888",  #Residencia en otro país
                                              .$ENT_MUN_RES_2010 %in% "997999" ~ '997999',
                                              .$ENT_PAIS_RES10 %in% "997" ~ "997",
                                              .$ENT_PAIS_RES10 %in% "998" ~ "998",
...
saveRDS(Migrantes, file = paste0(here::here(), "/Output/Municipio/01_Migracion reciente/2015/Matriz de migracion reciente a nivel municipal 2015.RDS"))
save(Migrantes, file = paste0(here::here(), "/Output/Municipio/01_Migracion reciente/2015/Matriz de migracion reciente a nivel municipal 2015.RData"))

require(openxlsx)
wb <- createWorkbook()
addWorksheet(wb, "M.reciente")
writeData(wb, 1, Migrantes %>% as.data.frame() %>% tibble::rownames_to_column(var = "CVE_MUN"), colNames = TRUE)
saveWorkbook(wb, file = paste0(here::here(), "/Bases/Municipio/01_Migracion reciente/2015/Matriz de migracion reciente a nivel municipal 2010 - 2015.xlsx"), overwrite = TRUE)
```
...
::: {style="height:500px;overflow:auto;"}
```{r, echo = FALSE}
load(paste0(here::here(), "/Output/Municipio/01_Migracion reciente/2015/Matriz de migracion reciente a nivel municipal 2015.RData"))
...
Librerias que se usaron en el documCVE_ENTo
