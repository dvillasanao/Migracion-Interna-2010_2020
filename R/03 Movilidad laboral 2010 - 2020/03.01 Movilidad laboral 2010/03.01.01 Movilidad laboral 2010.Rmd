...
```{r, echo = FALSE}
load(paste0(here::here(), "/Bases/03_Movilidad laboral_2010.RData"))
```
...
write.xlsx(tabla, file = paste0(here::here(),"/Output/Estado/03_Movilidad laboral/2010/Indicadores de movilidad laboral a nivel estatal 2010.xlsx"), overwrite = TRUE)

save(tabla, file = paste0(here::here(),"/Output/Estado/03_Movilidad laboral/2010/Indicadores de movilidad laboral a nivel estatal 2010.RData"))
```

```{r, echo = FALSE}
load(file = paste0(here::here(),"/Output/Estado/03_Movilidad laboral/2010/Indicadores de movilidad laboral a nivel estatal 2010.RData"))
...
Pob.ocupada <- mydata %>%
                mutate(CVE_MUN_TRAB = paste0(ENT_PAIS_TRAB, MUN_TRAB)) %>%
                 mutate(ENT_PAIS_TRAB = case_when(.$ENT_PAIS_TRAB %in% estados ~.$ENT_PAIS_TRAB,
                                                  ...
                                                  .$ENT_PAIS_TRAB %in% "997" ~ "999"),
                        CVE_MUN_TRAB = case_when(.$CVE_MUN_TRAB %in% municipios ~.$CVE_MUN_TRAB,
                                                 #### Se excluyen los municipios que no fueron muestreados
                                                  nchar(.$CVE_MUN_TRAB) == 3 ~ "888",  #Residencia en otro país
                                                 .$CVE_MUN_TRAB %in% "997999" ~ '997999',
                                                 .$ENT_PAIS_TRAB %in% "997" ~ "997",
                                                 .$ENT_PAIS_TRAB %in% "998" ~ "998",
                  ...
                  mutate(EDAD = as.numeric(.$EDAD)) %>%
                   subset((EDAD >= 15 & EDAD <=130) & (CONACT >= 10 & CONACT <= 20)) %>%
                    select(FACTOR, ESTRATO, UPM, CVE_MUN, CVE_MUN_TRAB, EDAD, CONACT) %>%
                     filter(CVE_MUN_TRAB %in% municipios) 

MC <- Pob.ocupada %>%
       svydesign(data = ., id = ~ UPM, strata = ~ESTRATO, weight = ~FACTOR, nest = T)
saveRDS(MC, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/MC_municipio.RDS"))
```


```{r, eval = FALSE}
MC <- readRDS(file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/MC_municipio.RDS"))
Migrantes <- svytable(~CVE_MUN_TRAB + CVE_MUN, design = MC) 
```
...
Migrantes <- Migrantes %>%
              as.data.frame() %>%
               expss::cross_cases(CVE_MUN, CVE_MUN_TRAB, weight = Freq) %>%
                as.data.frame() %>%
...
colnames <- names(Migrantes) %>% 
             as.data.frame() %>% 
              slice(-1) %>% 
               rename("CVE_MUN" = ".") %>%
                mutate(`CVE_MUN` = substr(.$CVE_MUN, 14, 19)) %>%
                 pull(CVE_MUN)
...
saveRDS(Migrantes, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/Matriz de migracion laboral a nivel municipal 2010.RDS"))
save(Migrantes, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/Matriz de migracion laboral a nivel municipal 2010.RData"))

require(openxlsx)
wb <- createWorkbook()
addWorksheet(wb, "M.Laboral")
writeData(wb, 1, Migrantes %>% as.data.frame() %>% tibble::rownames_to_column(var = "CVE_MUN"), colNames = TRUE)
saveWorkbook(wb, file = paste0(here::here(),"/Bases/Municipio/03_Movilidad laboral/2010/Matriz de migracion laboral a nivel municipal.xlsx"), overwrite = TRUE)
```


**Matriz de migración laboral a nivel municipal**  


<div style="height:500px;overflow:auto;">
```{r, echo = FALSE}
load(paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/Matriz de migracion laboral a nivel municipal 2010.RData"))
...
tabla <- Migrantes %>%
          as.data.frame() %>%
           tibble::rownames_to_column(var = "CVE_MUN") %>%
            melt(., id.vars = "CVE_MUN", variable.name = "CVE_MUN_TRAB") %>%
...
tabla1 <-  lapply(1:32, function(x){
                   tabla %>%
                    ...
                    mutate(CVE_MUN = case_when(substr(.$CVE_MUN, 1, 3) %in% estados[x] ~ .$CVE_MUN,
                           ...
                           CVE_MUN_TRAB = case_when(substr(.$CVE_MUN_TRAB, 1, 3) %in% estados[x] ~ .$CVE_MUN_TRAB,
                                          substr(.$CVE_MUN_TRAB, 1, 3) %nin% estados[x] ~ substr(.$CVE_MUN_TRAB, 1, 3)))  %>%
                    # Se seleccionan los valores de la entidad seleccionada
                     mutate(value = ifelse(substr(.$CVE_MUN, 1, 3) %in% estados[x] | substr(.$CVE_MUN_TRAB, 1, 3) %in% estados[x], value, 0)) %>%
                      # Se cambian las etiquetas a los estados 
                      mutate(CVE_MUN = case_when(substr(.$CVE_MUN, 1, 3) %in% estados[x] ~ .$CVE_MUN,
                                            substr(.$CVE_MUN, 1, 3)  %nin% estados[x] ~ paste0(nom_estados[as.numeric(substr(.$CVE_MUN, 1, 3))])),
                             CVE_MUN_TRAB = case_when(substr(.$CVE_MUN_TRAB, 1, 3) %in% estados[x] ~ .$CVE_MUN_TRAB,
                                                                substr(.$CVE_MUN_TRAB, 1, 3)  %nin% estados[x] ~ paste0(nom_estados[as.numeric(substr(.$CVE_MUN_TRAB, 1, 3))]))) %>%
                      ################################## Filtro ##############################
                      # Se hace un filtro de los flujos migratorios arriba de 1 000 habitantes
                       mutate(value = ifelse((.$CVE_MUN != .$CVE_MUN_TRAB) & .$value >= 500, value, 0)) %>%
                        dcast(., CVE_MUN ~ CVE_MUN_TRAB, value.var = "value", sum,  na.rm = TRUE) %>%
                         column_to_rownames(., var = "CVE_MUN") 
  }
)

saveRDS(tabla1, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/Tablas a nivel municipal_chordDiagram.RDS"))
```
...
tabla2 <- lapply(1:32, function(x){
                  ...
                  circos.clear()
                  chordDiagramFromMatrix(tabla1[[x]] %>% as.matrix(), 
                                         grid.col = groupColors)  %>%
...
saveRDS(tabla2, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2010/Paleta de colores a nivel municipal_chordDiagram.RDS"))
                ...
                tibble::rownames_to_column(var = "CVE_MUN") %>%
                 melt(., id.vars = "CVE_MUN", variable.name = "CVE_MUN_TRAB") %>%
                  mutate_at(vars(3), as.numeric) %>%
                   as_tibble() %>%
                    filter(CVE_MUN != CVE_MUN_TRAB) %>%
                     group_by(CVE_MUN_TRAB) %>%
                      summarise(Emigrantes = sum(value, na.rm = TRUE)) %>%
                       rename("CVE_MUN" = "CVE_MUN_TRAB") 
...
write.xlsx(tabla, file = paste0(here::here(),"/Output/Municipio/03_Movilidad laboral/2010/Indicadores de migracion laboral a nivel municipal.xlsx"), overwrite = TRUE)

save(tabla, file = paste0(here::here(),"/Output/Municipio/03_Movilidad laboral/2010/Indicadores de migracion laboral a nivel municipal.RData"))
```


```{r, echo = FALSE}
load(file = paste0(here::here(),"/Output/Municipio/03_Movilidad laboral/2010/Indicadores de migracion laboral a nivel municipal.RData"))
