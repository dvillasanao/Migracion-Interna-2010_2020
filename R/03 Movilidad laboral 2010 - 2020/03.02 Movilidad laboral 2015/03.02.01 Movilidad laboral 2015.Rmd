...
```{r}
load(file = paste0(here::here(), "/Bases/03_Migracion laboral_2015.RData"))
...
write.xlsx(tabla, file = paste0(here::here(),"/Output/Estado/03_Movilidad laboral/2015/Indicadores de movilidad laboral a nivel estatal 2015.xlsx"), overwrite = TRUE)

save(tabla, file = paste0(here::here(),"/Output/Estado/03_Movilidad laboral/2015/Indicadores de movilidad laboral a nivel estatal 2015.RData"))
```

```{r, echo = FALSE}
load(file = paste0(here::here(),"/Output/Estado/03_Movilidad laboral/2015/Indicadores de movilidad laboral a nivel estatal 2015.RData"))
...
MC <- Pob.ocupada %>%
       svydesign(data = ., id = ~ UPM, strata = ~ESTRATO, weight = ~FACTOR, nest = T)
saveRDS(MC, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/MC_municipio.RDS"))
```


```{r, eval = FALSE}
MC <- readRDS(file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/MC_municipio.RDS"))
...
saveRDS(Migrantes, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/Matriz de migracion laboral a nivel municipal 2015.RDS"))
save(Migrantes, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/Matriz de migracion laboral a nivel municipal 2015.RData"))

require(openxlsx)
wb <- createWorkbook()
addWorksheet(wb, "M.Laboral")
writeData(wb, 1, Migrantes %>% as.data.frame() %>% tibble::rownames_to_column(var = "CVE_MUN"), colNames = TRUE)
saveWorkbook(wb, file = paste0(here::here(),"/Bases/Municipio/03_Movilidad laboral/2015/Matriz de migracion laboral a nivel municipal.xlsx"), overwrite = TRUE)
```
...
<div style="height:500px;overflow:auto;">
```{r, echo = FALSE}
load(paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/Matriz de migracion laboral a nivel municipal 2015.RData"))
...
tabla1 <-  lapply(1:32, function(x){
              ...
              # Se reestructura la clave de municipios y se seleccionan solo los municipios del filtro del estado
                    mutate(CVE_MUN = case_when(substr(.$CVE_MUN, 1, 3) %in% estados[x] ~ .$CVE_MUN,
                           ...
                           CVE_MUN_TRABAJO = case_when(substr(.$CVE_MUN_TRABAJO, 1, 3) %in% estados[x] ~ .$CVE_MUN_TRABAJO,
                                          substr(.$CVE_MUN_TRABAJO, 1, 3) %nin% estados[x] ~ substr(.$CVE_MUN_TRABAJO, 1, 3)))  %>%
              # Se seleccionan los valores de la entidad seleccionada
                     mutate(value = ifelse(substr(.$CVE_MUN, 1, 3) %in% estados[x] | substr(.$CVE_MUN_TRABAJO, 1, 3) %in% estados[x], value, 0)) %>%
              # Se cambian las etiquetas a los estados 
                      mutate(CVE_MUN = case_when(substr(.$CVE_MUN, 1, 3) %in% estados[x] ~ .$CVE_MUN,
                                            substr(.$CVE_MUN, 1, 3)  %nin% estados[x] ~ paste0(nom_estados[as.numeric(substr(.$CVE_MUN, 1, 3))])),
                             CVE_MUN_TRABAJO = case_when(substr(.$CVE_MUN_TRABAJO, 1, 3) %in% estados[x] ~ .$CVE_MUN_TRABAJO,
                                                                substr(.$CVE_MUN_TRABAJO, 1, 3)  %nin% estados[x] ~ paste0(nom_estados[as.numeric(substr(.$CVE_MUN_TRABAJO, 1, 3))]))) %>%
              ################################## Filtro ##############################
              # Se hace un filtro de los flujos migratorios arriba de 1 000 habitantes
                       mutate(value = ifelse((.$CVE_MUN != .$CVE_MUN_TRABAJO) & .$value >= 500, value, 0)) %>%
                        dcast(., CVE_MUN ~ CVE_MUN_TRABAJO, value.var = "value", sum,  na.rm = TRUE) %>%
                         column_to_rownames(., var = "CVE_MUN") 
  }
)

saveRDS(tabla1, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/Tablas a nivel municipal_chordDiagram.RDS"))
```
...
saveRDS(tabla2, file = paste0(here::here(), "/Output/Municipio/03_Movilidad laboral/2015/Paleta de colores a nivel municipal_chordDiagram.RDS"))
...
write.xlsx(tabla, file = paste0(here::here(),"/Output/Municipio/03_Movilidad laboral/2015/Indicadores de migracion laboral a nivel municipal.xlsx"), overwrite = TRUE)

save(tabla, file = paste0(here::here(),"/Output/Municipio/03_Movilidad laboral/2015/Indicadores de migracion laboral a nivel municipal.RData"))
```


```{r, echo = FALSE}
load(file = paste0(here::here(),"/Output/Municipio/03_Movilidad laboral/2015/Indicadores de migracion laboral a nivel municipal.RData"))
